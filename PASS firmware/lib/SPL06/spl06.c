...
int read(char *path, uint8_t *buff, int len)
{
    return 0;
}

int write(char *path, uint8_t *buff, int len)
{
   return 0; 
}
...
struct {
    ...
    int t_peroid;  // measurement period
    int t_ready;   // measurement finished

    int inited;

} _spl;

const float ScaleMap[] = 
{
    524288.0f,
    1572864.0f,
    3670016.0f,
...
{
    int log2 = 0;
    for(; log2 < 7 && oversampling_rate > 0; log2++)
        oversampling_rate >>= 1;

    return ScaleMap[log2];
}
